...
<section class="faqs faqs_screen" >
	...
	<div class="container">
		...
		<h2 class="title_h2 text-center none_mobile">Frequently Asked <br> Questions <span>?</span></h2>
		...
		<div class="row">
			...
			<div class="col-sm-4">
                ...
                <ul class="tabs_navi nav none_mobile">
                ...
                <?php 
 				...
 				<select class="form-select form_select_tabs" id="faqCategoryDropdown" style="display: none;">
    <?php 
        $sqlCategory = "SELECT * FROM tbl_faq_categories WHERE faq_categories_status = 1";
        $resCategory = $database->get_results($sqlCategory);
        ...
        for ($c = 0; $c < count($resCategory); $c++) {
            $rowCategory = $resCategory[$c];
    ?>
        <option value="category<?php echo $rowCategory['faq_categories_id']; ?>" <?php if ($c == 0) echo 'selected'; ?>>
            <?php echo $rowCategory['faq_categories_name']; ?>
        </option>
    <?php } ?>
</select>
...
<script>
    document.getElementById('faqCategoryDropdown').addEventListener('change', function () {
        // Get selected value
        ...
        const activeTab = document.getElementById(selectedCategory);
        if (activeTab) {
