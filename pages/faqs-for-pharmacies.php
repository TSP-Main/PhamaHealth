		...
		<div class="row">
			...
			<div class="col-sm-4">
                ...
                <?php 
 				...
 				<select id="faqCategories" class="form-select form_select_tabs" style="display: none;">
    <?php 
        $sqlCategory = "select * from tbl_faq_categories_pharmacy where faq_categories_status=1 order by faq_categories_id";
        $resCategory = $database->get_results($sqlCategory);
        if ($resCategory) {
            for ($c = 0; $c < count($resCategory); $c++) {
                $rowCategory = $resCategory[$c];
                ?>
                <option value="category<?php echo $rowCategory['faq_categories_id']; ?>" 
                    <?php if ($c == 0) echo 'selected'; ?>>
...
<script>
  document.getElementById('faqCategories').addEventListener('change', function () {
    ...
    // Add 'active' and 'show' classes to the selected tab pane
